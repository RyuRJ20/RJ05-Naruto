...
public class FireballEntity extends AbstractArrow implements ItemSupplier {
	public FireballEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(RjsNarutoModEntities.FIREBALL.get(), world);
	}
	...
	public static FireballEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
		FireballEntity entityarrow = new FireballEntity(RjsNarutoModEntities.FIREBALL.get(), entity, world);
	...
	public static FireballEntity shoot(LivingEntity entity, LivingEntity target) {
		FireballEntity entityarrow = new FireballEntity(RjsNarutoModEntities.FIREBALL.get(), entity, entity.level);
		...
		entity.level.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.fire.ambient")), SoundSource.PLAYERS, 1, 1f / (new Random().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
